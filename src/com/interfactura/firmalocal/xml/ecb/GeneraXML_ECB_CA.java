...
import com.interfactura.firmalocal.xml.WebServiceClienteCaptacion;
...
@Component
public class GeneraXML_ECB_CA {
	private Logger logger = Logger.getLogger(GeneraXML_ECB_CA.class);
	private BufferedReader br;
	private String linea;
	private String token;
	@Autowired
	private Convertir_CA conver;
	private int cont;
	@Autowired
	...
	private WebServiceClienteCaptacion servicePort = null;
    ...
    private List<MovimientoECB_CA> lstMovimientosECB = new ArrayList<MovimientoECB_CA>();						
	private EstadoDeCuentaBancario_CA estadoDeCuentaBancario = new EstadoDeCuentaBancario_CA();
    ...
    private List<ECB_CA> lstObjECBs = new ArrayList<ECB_CA>();
  ...
  //Banderas para comprobar que existan algunos atributos de Santander:Movimientos
	 ...
	 * @param nameFile
	 */
	public GeneraXML_ECB_CA(String nameFile) {
		...
		try{
			...
			*/
			...
			if(this.servicePort == null){
				this.servicePort = new WebServiceClienteCaptacion();								
			}			
			...
			System.out.println("TipoFormato:" + 0);
...
//			}
	...
	{	//System.out.println("formatLinea");
		...
		case 1:
			...
			//conver.set(linea, contCFD, fileNames);
						...
						if(root.getChildNodes().item(i).getChildNodes().item(x) instanceof Element && 
							...
							for(int j=0; j<root.getChildNodes().item(i).getChildNodes().item(x).getChildNodes().getLength(); j++){
								...
								if(root.getChildNodes().item(i).getChildNodes().item(x).getChildNodes().item(j) instanceof Element && 
									...
									EstadoDeCuentaBancario_CA ecb = new EstadoDeCuentaBancario_CA();
											...
											if(root.getChildNodes().item(i).getChildNodes().item(x).getChildNodes().item(j).getChildNodes().item(k) instanceof Element && 
												...
												for(int l=0; l<root.getChildNodes().item(i).getChildNodes().item(x).getChildNodes().item(j).getChildNodes().item(k).getChildNodes().getLength(); l++){
													...
													if(root.getChildNodes().item(i).getChildNodes().item(x).getChildNodes().item(j).getChildNodes().item(k).getChildNodes().item(l) instanceof Element){
														MovimientoECB_CA movEcb = new MovimientoECB_CA();	
	...
	private Document putMovimientoECB(Element docEleComprobante, Document domResultado, EstadoDeCuentaBancario_CA estadoDeCuentaBancariox, List<MovimientoECB_CA> lstMovimientosECBx){
							...
							if(nodo2 instanceof Element && nodo2.getNodeName().equals("Santander:EstadoDeCuentaBancario")){
								...
								for(int k=0; k<hijosEstadoDeCuentaBancario.getLength(); k++){
									...
									if(nodo3 instanceof Element && nodo3.getNodeName().equals("Santander:Movimientos")){
										...
										MovimientoECB_CA [] arrayMov = new MovimientoECB_CA[lstMovimientosECBx.size()];
					...
					{					
						...
						try{
							...
							//if(lstObjECBs.size()<26643 ){
								...
								//Validar que el nombre de aplicativo informado en cada comprobante, sea correcto
								if(conver.getTags().NOMBRE_APP_REPECB.equals(""))
									throw new Exception(
											"El nombre de Aplicativo no existe para la interface " + fileNames);
								
								//ecbActual++;
								ECB_CA objEcbActual = new ECB_CA();
								this.lstMovimientosECB = new ArrayList<MovimientoECB_CA>();						
								this.estadoDeCuentaBancario = new EstadoDeCuentaBancario_CA();
	 ...
	 */
	public void fileSALIDA(ECB_CA objECB, String fileNames, String strUUID, String strFechaTimbrado, String strNoCertificadoSAT, String strSelloCFD,
			String strSelloSAT, String strVersion, String strEmisorRFC, String strReceptorRFC, String strTotalZeros) 
		throws IOException, ParseException 
	{/*
		DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
	...
	public void setBDv2(String strUUID, ECB_CA objECB) 
		...
		throws IOException 
	{
		...
		long t1 = System.currentTimeMillis();
		temp = NombreAplicativo_CA.obtieneNombreApp(getNombresApps(), strFileNames, numeroMalla) + "|CFD|" + strTagEMISON_RFC 
			+ "|" + strTagNUM_CTE
			+ "|" + strTagNUM_CTA
