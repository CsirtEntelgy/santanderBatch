 ...
 */
@Component
public class Convertir_NC 
{

	private Logger logger = Logger.getLogger(Convertir_NC.class);
	...
	public void set(String linea, long numberLine) 
	{
		lineas = linea.split("\\|");
	 ...
	 */
	public byte[] fComprobante(String linea, long numberLine, HashMap<String, HashMap> tipoCambio, HashMap fiscalEntities, HashMap campos22) 
		throws UnsupportedEncodingException 
	{
			...
			if (lineas.length >= 8) 
			{
				System.out.println("antesConcat");
				concat = new StringBuffer();
				
				tags.CFD_TYPE = lineas[1].trim();
				
				tags.SERIE_FISCAL_CFD = lineas[3].trim();
				System.out.println("despuesSERIE_FISCAL_CFD " + tags.SERIE_FISCAL_CFD );
				if (!Util.isNullEmpty(tags.SERIE_FISCAL_CFD)) 
				{
					System.out.println("despuesUtil.isNullEmpty(tags.SERIE_FISCAL_CFD) " + tags.SERIE_FISCAL_CFD );
					if(tags.SERIE_FISCAL_CFD.length() >= 3){
						if(tags.SERIE_FISCAL_CFD.substring(0, 3).trim().equals("MXN") || 
							...
							concat.append(" MonedaIncorrecta-" + tags.SERIE_FISCAL_CFD + "\"");
						}
		...
		throws UnsupportedEncodingException 
	{
		...
		if (lineas.length >= 5) {
			return Util
					.conctatArguments(
							"\n<ecb:EstadoDeCuentaBancario version=\"1.0\" ",
							"numeroCuenta=\"", lineas[1].trim(), "\" ",
							"nombreCliente=\"", Util.convierte(lineas[2].trim()), "\" ",
		...
		throws UnsupportedEncodingException 
	{
			...
			if (lineas.length >= 7) 
			{
				...
				if ((rfcEnajenante != null)&&(rfcEnajenante.trim().length() > 0) && flgRfcOk)
				{
					...
					return Util
					.conctatArguments("\n<ecb:MovimientoECBFiscal ", fechaCal,
