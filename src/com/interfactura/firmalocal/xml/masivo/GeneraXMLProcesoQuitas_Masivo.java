...
@Component
public class GeneraXMLProcesoQuitas_Masivo {
	private Logger logger = Logger.getLogger(GeneraXMLProcesoQuitas_Masivo.class);
	...
	private TagsXML tags;
   	...
   	public GeneraXMLProcesoQuitas_Masivo() {
	...
	public void processControlFile(String urlWSTimbrado){
		...
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(PathQuitasProceso + "massiveProcessQuitas.txt");
			fileStatus.write(("Status del proceso bash massiveProcessQuitas.sh" + "\n").getBytes("UTF-8"));
			while((strLine = br.readLine()) != null){
				...
				if(arrayRenglon.length>1){
					...
					File fileTXT = new File(PathQuitasProceso + arrayRenglon[1] + "/" + nameFileExcel);
					if(!fileTXT.exists()){
						fileStatus.write(("El archivo " + nameFileExcel + " no se encuentra en la ruta " + PathQuitasProceso + arrayRenglon[1] + "/" + "\n").getBytes("UTF-8"));
						userlog = new FileOutputStream(PathQuitasSalida + arrayRenglon[1] + "/LOG" + arrayRenglon[1] + ".TXT",true);
						userlog.write(("El archivo " + nameFileExcel + " no se encuentra en la ruta " + PathQuitasProceso + arrayRenglon[1] + "/" + "\r\n").getBytes("UTF-8"));
					}else{												
						this.offSetComprobante = 0;
						
						String strAbsolutePathBD = PathQuitasProceso + arrayRenglon[1] + "/" + "BD" + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
						...
						FileInputStream fisTxt = new FileInputStream(PathQuitasProceso + arrayRenglon[1] + "/" + nameFileExcel);
							...
							if(!strLineTXT.equals("")){
								if(arrayValues.length<49){							
									...
									if(sbErrorFile.toString().length() > 0){
										...
										System.out.println("fError EmisionMasivaQuitas: " + sb.toString());							
									...
									if(sbErrorFile.toString().length() > 0 ){
										...
										for(int index=0; index<listIn.size(); index++){
											if(listIn.get(index).getByteArrXMLSinAddenda() != null){
												...
												xmlTimbradoConPipe = this.servicePort.generaTimbre(strXmlATimbrar, false, urlWSTimbrado, properties, nameFileExcel.substring(0, nameFileExcel.indexOf(".")), 0, 1, listIn.get(index).getPeriod(), "");
												...
												if(strDescripcion.toLowerCase().trim().equals("ok") && strIdRespuesta.equals("1")) {
													...
													String strAbsolutePathOndemand = PathQuitasOndemand + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + "/";
						...
						String strAbsolutePathXML = PathQuitasSalida + arrayRenglon[1] + "/" + "XML" + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
						String strAbsolutePathINC = PathQuitasSalida + arrayRenglon[1] + "/" + "INC" + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
			...
			try {
				...
				FileOutputStream fileError = new FileOutputStream(PathQuitasProceso + "massiveProcessError.txt");
