...
import com.interfactura.firmalocal.xml.util.UtilCFDIComplementoPago;
...
@Component
public class GeneraXMLComplementoPagoProceso_Masivo {
	private Logger logger = Logger.getLogger(GeneraXMLComplementoPagoProceso_Masivo.class);
	...
	// private static final String RE_DECIMAL_NEGATIVO =
	// "[\\-]?[0-9]+(\\.[0-9][0-9]?[0-9]?[0-9]?)?";
	...
	@Autowired(required = true)
	private UtilCFDIComplementoPago fillComplementoPago;

	@Autowired
	private TagsXML tags;

	public GeneraXMLComplementoPagoProceso_Masivo() {
	...
	public void processControlFile(String urlWSTimbrado) {
		...
		try {
			...
			// FileInputStream fsExcelsToProcess = new
			// FileInputStream(properties.getPathFacturacionEntrada() + "IDFILEPROCESS_" +
			// nProceso + ".TXT");
			...
			for (Customer customer : customerManager.listar()) {
				...
				if (customerNew.getTaxId().toUpperCase().trim().equals("XEXX010101000")
						|| customerNew.getTaxId().toUpperCase().trim().equals("XAXX010101000")) {
					// Cliente extranjero
			...
			// FileOutputStream fileStatus = new FileOutputStream(PathFacturacionProceso +
			// "STATUS_PROCESS_" + nProceso + ".TXT");
			...
			while ((strLine = br.readLine()) != null) {
				...
				if (arrayRenglon.length > 1) {
					...
					if (!fileTXT.exists()) {
						fileStatus.write(("El archivo " + nameFileExcel + " no se encuentra en la ruta "
								+ PathFacturacionProceso + arrayRenglon[1] + "/" + "\n").getBytes("UTF-8"));
						userlog = new FileOutputStream(
								PathFacturacionSalida + arrayRenglon[1] + "/LOG" + arrayRenglon[1] + ".TXT", true);
						userlog.write(("El archivo " + nameFileExcel + " no se encuentra en la ruta "
								+ PathFacturacionProceso + arrayRenglon[1] + "/" + "\r\n").getBytes("UTF-8"));
					} else {
						// Crear directorio en ruta ../facturacion/salida/
						/*
						 * File fileDirectory = new File(PathFacturacionSalida + arrayRenglon[1] + "/");
						 * if(!fileDirectory.exists()){ fileDirectory.mkdir(); }else{ for(File
						 * file:fileDirectory.listFiles()){ file.delete(); } }
						 */

						this.offSetComprobante = 0;
						// String strAbsolutePathXML = PathFacturacionSalida + arrayRenglon[1] + "/" +
						// "XML" + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
						String strAbsolutePathBD = PathFacturacionProceso + arrayRenglon[1] + "/" + "BD"
								+ nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";

						// String strAbsolutePathODM = properties.getPathFacturacionOndemand() + "ODM-"
						// + nameFileExcel.substring(0, nameFileExcel.indexOf("."));
						// String strAbsolutePathINC = PathFacturacionSalida + arrayRenglon[1] + "/" +
						// "INC" + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
						...
						// StringBuilder sbErrorFile =
						// this.processExcelFile("/planCFD/procesos/Interfactura/interfaces/" +
						// nameFileExcel);

						// processOneSheet("/planCFD/procesos/Interfactura/interfaces/" +
						// nameFileExcel);

						FileInputStream fisTxt = new FileInputStream(
								PathFacturacionProceso + arrayRenglon[1] + "/" + nameFileExcel);
						...
						while ((strLineTXT = brTXT.readLine()) != null) {
							...
							if (!strLineTXT.equals("")) {
								if (arrayValues.length < 55) {
									// System.out.println("Factura " + (factura+1) + ", incompleta!");
									// salidaINC.write(("ErrorArchivo|" + "Factura " + (factura+1) + ", incompleta!"
									// + "\n").getBytes("UTF-8"));

									sbStatusNoOK.append(
											"ErrorArchivo|" + "Factura " + (factura + 1) + ", incompleta!" + "\n");

									counterNoOk++;

								} else if (!arrayValues[arrayValues.length - 1].trim().toUpperCase()
										.equals("FINFACTURA")) {
									System.out.println(
											"Estructura de Archivo incorrecta, no fue encontrada la etiqueta de control ||FINFACTURA|| en la factura "
													+ (factura + 1));
									// salidaINC.write(("ErrorArchivo|" + "Estructura de Archivo incorrecta, no fue
									// encontrada la etiqueta de control ||FINFACTURA|| en la factura " +
									// (factura+1) + "\n").getBytes("UTF-8"));
									sbStatusNoOK.append("ErrorArchivo|"
											+ "Estructura de Archivo incorrecta, no fue encontrada la etiqueta de control ||FINFACTURA|| en la factura "
											+ (factura + 1) + "\n");
									...
									// StringBuilder sbErrorFile = this.processRowExcel(arrayValues, factura+1,
									// hashIvas, hashcodigoISO, hashmoneda, hashEmisores, hashClientes,
									// hashCfdFieldsV22);
									StringBuilder sbErrorFile = new StringBuilder();

									// llenar comprobante desde archivo
									comp = fillComplementoPago.fillComprobanteComplementoPagoFromTxt(arrayValues);
									...
									if (sbErrorFile.toString().length() > 0) {
										...
										try {
											...
											Document document = UtilCatalogos.convertStringToDocument(
													invoice.getByteArrXMLSinAddenda().toString("UTF-8"));
											String totalIvaRet = UtilCatalogos.getStringValByExpression(document,
											...
											String errors = UtilCatalogos.validateCfdiDocument(document,
											...
											sb.append("Factura: " + factura + " --Exception: " + "\n" + e.getMessage()
													+ "\n");
											System.out.println(
													"Factura: " + factura + " --Exception: " + "\n" + e.getMessage());
											invoice.setSbError(new StringBuilder(
													"Factura: " + factura + " --Exception: " + e.getMessage() + "\n"));
											listIn.add(invoice);
											listComprobantes.add(comp);
										}
									...
									if (sbErrorFile.toString().length() > 0) {
										...
										for (int index = 0; index < listIn.size(); index++) {
											if (listIn.get(index).getByteArrXMLSinAddenda() != null) {
												String strXmlATimbrar = listIn.get(index).getByteArrXMLSinAddenda()
														.toString("UTF-8");
												...
												// String helloMessage =
												// servicePort.consultaTimbre("F7EFFF4D-2816-425C-81CC-B26DF8C177DB",
												// null, true) ;
												// System.out.println("Conectandose...........");

												// System.out.println("nomInterface" + nameFileExcel.substring(0,
												...
												String xmlTimbrado = xmlTimbradoConPipe.substring(0,
														xmlTimbradoConPipe.length() - 1);
												...
												// Se verifica si la respuesta del web service es correcta, sino se
												// lanza excepci�n
												Element docEle = dom.getDocumentElement();
												String strDescripcion = docEle.getAttribute("Descripcion");
												String strIdRespuesta = docEle.getAttribute("IdRespuesta");
												if (strDescripcion.toLowerCase().trim().equals("ok")
														&& strIdRespuesta.equals("1")) {
													...
													// doc = this.generaXML.agregaAddenda(doc, invoice);//agrega addenda
													// anterior
													...
													String strXmlfinalConAddenda = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
															+ sbXmlFinal.toString().replaceAll("[\n\r]", "");

													// System.out.println("Guarda en XML...TXT");
													// salidaXML.write(("Factura " + (factura+1) + " correcta!" +
													// "\r\n").getBytes("UTF-8"));

													sbStatusOK.append("Factura " + (factura + 1)
															+ " correcta!, folioSAT:" + strUUID + "\r\n");

													counterOk++;

													// Guardar ruta y posicion inicio y fin
													long length = strXmlfinalConAddenda.getBytes("UTF-8").length;
													// listIn.get(index).setXmlRoute(strAbsolutePathODM + "|" +
													// this.offSetComprobante + "|" + (this.offSetComprobante +
													// length));

													// String strAbsolutePathXml = properties.getPathDirGenr() +
													// idMassive + nameFileExcel.substring(0,
													// nameFileExcel.indexOf(".")) + ".xml";
													String strAbsolutePathOndemand = PathFacturacionOndemand
															+ nameFileExcel.substring(0, nameFileExcel.indexOf("."))
															+ "/";
													...
													String strAbsolutePathXml = strAbsolutePathOndemand + "fac-"
															+ strUUID + ".xml";
													...
													listIn.get(index)
															.setXmlRoute(strAbsolutePathXml + "|" + 0 + "|" + 0);

													// System.out.println("Guarda en BD...TXT");
													salidaBD.write(this
															.buildRowBDFile(listIn.get(index), strUUID, nameFileExcel,
																	idUsuario, idArea, nombreUsuario)
															.getBytes("UTF-8"));
													...
													String temp = "ErrorFactura|" + (factura + 1) + "|" + nameFileExcel
															+ "|" + listIn.get(index).getFe_taxid() + "|"
															+ listIn.get(index).getRfc() + "|" + "\r\n";
													// salidaINC.write(temp.getBytes("UTF-8"));
													// salidaINC.write((strIdRespuesta + strDescripcion +
													// "\r\n").getBytes("UTF-8"));
													sbStatusNoOK
															.append(temp + strIdRespuesta + strDescripcion + "\r\n");
												...
												sbStatusNoOK.append(
														temp + listIn.get(index).getSbError().toString() + "\r\n");
						...
						String strAbsolutePathXML = PathFacturacionSalida + arrayRenglon[1] + "/" + "XML"
								+ nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
						String strAbsolutePathINC = PathFacturacionSalida + arrayRenglon[1] + "/" + "INC"
								+ nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
						...
						this.salidaXML.write(
								("--Total de facturas correctas: " + counterOk + " --Total de facturas procesadas: "
										+ counterProcesadas + "\r\n\r\n" + sbStatusOK.toString()).getBytes("UTF-8"));
						this.salidaINC.write(("--Total de facturas con incidentes: " + counterNoOk
								+ " --Total de facturas procesadas: " + counterProcesadas + "\r\n\r\n"
								+ sbStatusNoOK.toString()).getBytes("UTF-8"));
						System.out.println(("--Incidentes log: \nTotal de facturas con incidentes: " + counterNoOk
								+ " --Total de facturas procesadas: " + counterProcesadas + "\r\n\r\n"
								+ sbStatusNoOK.toString()).getBytes("UTF-8"));
						if (salidaXML != null)
							salidaXML.close();
			...
			try {
				// FileOutputStream fileError = new FileOutputStream(PathFacturacionProceso +
				// "ERROR_PROCESS_" + nProceso + ".TXT");
				...
				e1.printStackTrace();
				// System.out.println("Exception al crear ERROR_PROCESS_" + nProceso + ".TXT:" +
				// e1.getMessage());
	...
	public String buildRowBDFile(Invoice_Masivo invoiceM, String strUUID, String strSourceFileName, String idUsuario,
			String idArea, String nombreUsuario) {
	...
	public StringBuilder processRowExcel(String[] strValues, int factura, HashMap<Integer, String> hashIvas,
			HashMap<String, String> hashcodigoISO, HashMap<String, String> hashmoneda,
			HashMap<String, FiscalEntity> hashEmisores, HashMap<String, Customer> hashClientes,
		...
		try {
			 ...
			 * System.out.
			 * println("Estructura de Archivo incorrecta, no fue encontrada la etiqueta de control ||FINFACTURA||"
			 * ); sbErrorFile = new StringBuilder(); sbErrorFile.
			 * append("Estructura de Archivo incorrecta, no fue encontrada la etiqueta de control ||FINFACTURA||"
			 * ); }else if(strValues.length<30){ System.out.println("Factura " + factura +
			 * ", incompleta!"); sbErrorFile = new StringBuilder();
			 * sbErrorFile.append("Factura " + factura + ", incompleta!"); }else{
			 */
			fError = false;
			...
			if (strValues[2] == null) {
				...
				} else if (strValues[2].toString().trim().toUpperCase().equals("INGRESO")
						|| strValues[2].toString().trim().toUpperCase().equals("EGRESO")) {
			...
			if (strValues[5] == null) {
				...
				if (invoice.getRfc().trim().equals("")) {
					...
					if (!invoice.getRfc().toUpperCase().equals("XEXX010101000")
							&& !invoice.getRfc().toUpperCase().equals("XAXX010101000")) {
						if (fiscalEntity != null) {
							System.out.println("CustomerRFC: " + invoice.getRfc() + " feId: "
						...
						if (strValues[6] == null || strValues[6].toString().trim().length() == 0) {
							...
							if (validaDatoRE(strValues[6].toString().trim(), RE_CHARNUMBER_IDEXT)) {
								...
								System.out.println(
										"(7) Id extranjero con formato incorrecto - Factura " + factura + "\n");
								fError = true;
							}
						}
					}
			...
			if (customer != null) {
				...
				if (customer.getAddress() == null) {
					...
					if (customer.getAddress().getInternalNumber() == null) {
						...
						// sbError.append("Num Interior del Cliente no existe en BD - Factura " +
						// factura + "\n");
						// System.out.println("Num Interior del Cliente no existe en BD - Factura " +
						// factura + "\n");
						// fError = true;
						invoice.setInterior("");
					} else {
						// System.out.println("Numero interno: " +
						// customer.getAddress().getInternalNumber());
					...
					if (customer.getAddress().getExternalNumber() == null) {
						...
						// customer.getAddress().getExternalNumber());
					...
					if (customer.getAddress().getState() == null) {
						...
						if (customer.getAddress().getState().getCountry() == null) {
							...
							if (customer.getAddress().getState().getCountry().getName() == null) {
								...
								// customer.getAddress().getState().getCountry().getName());
				...
				if ((!invoice.getRfc().toUpperCase().equals("XEXX010101000")
						&& !invoice.getRfc().toUpperCase().equals("XAXX010101000"))
						|| ((invoice.getRfc().toUpperCase().equals("XEXX010101000")
								|| invoice.getRfc().toUpperCase().equals("XAXX010101000"))
			...
			if (strValues[9] == null) {
				...
				System.out
						.println("(10) Posicion Numero de Cuenta de Pago requerida (Null) - Factura " + factura + "\n");
				fError = true;
			} else {
				if (strValues[9].toString().trim().equals("")) {
					...
					if (validaDatoRE(strNumCtaPago, RE_CHAR_ALL)) {
						...
						/*
						 * if(strNumCtaPago.length() == 4){ invoice.setNumCtaPago(strNumCtaPago);
						 * //System.out.println("Numero de cuenta de pago: " + strNumCtaPago); }else{
						 * sbError.
						 * append("(10) Numero de Cuenta de Pago debe ser de 4 digitos - Factura " +
						 * factura + "\n"); System.out.
						 * println("(10) Numero de Cuenta de Pago debe ser de 4 digitos - Factura " +
						 * factura + "\n"); fError = true; }
						 */
						...
						sbError.append(
								"(10) Numero de Cuenta de Pago con formato incorrecto - Factura " + factura + "\n");
						System.out.println(
								"(10) Numero de Cuenta de Pago con formato incorrecto - Factura " + factura + "\n");
						fError = true;
				...
				if (strValues[11].toString().trim().equals("")) {
					...
					if (strValues[11].toString().trim().length() > 0) {
						...
						if (validaDatoRELongitud(strCodigoCliente, "[a-zA-Z0-9&_\\/\\-\\s]*", 10)) {
							...
							System.out
									.println("(12) Codigo Cliente con formato incorrecto - Factura " + factura + "\n");
							fError = true;
						}
					} else {
				...
				if (strValues[14].toString().trim().equals("")) {
					...
					if (strValues[14].toString().trim().length() > 0) {
						...
						if (validaDatoRELongitud(strCentroCostos, "[a-zA-Z0-9&_\\/\\-\\s]*", 20)) {
							...
							System.out.println(
									"(15) Centro de Costos con formato incorrecto - Factura " + factura + "\n");
							fError = true;
						}
					} else {
					...
					if (strTipoAddenda.equals("1")) {
						...
						if (strValues[19] == null) {
							...
							if (!strValues[19].toString().trim().equals("")) {
								...
								// System.out.println("codigoISO Log: " +
								// strValues[19].toString().trim().toUpperCase());
							} else {
						...
						if (strValues[19] == null) {
							...
							if (!strValues[19].toString().trim().equals("")) {
								...
								// System.out.println("codigoISO Arr: " +
								// strValues[19].toString().trim().toUpperCase());
							} else {
								sbError.append("(20) Codigo ISO Moneda no encontrado" + "\n");
								System.out.println("(20) Codigo ISO Moneda no encontrado" + "\n");
				...
				if (strValues[31] == null) {
					...
					if (strValues[31].toString().trim().equals("")) {
						...
						if (validaDatoRE(strValues[31].toString().trim(), RE_DECIMAL_NEGATIVO)) {
							...
							fError = true;
							sbError.append(
									"(32) " + "Descuento con formato incorrecto " + " - factura " + factura + "\n");
						}
					}
				}

			} else {
				if (strValues[30].toString().trim() != null && strValues[30].toString().trim().length() > 0
						&& strValues[30].toString().trim().length() <= 1500) {
					...
					if (strValues[31] == null) {
						...
						fError = true;
						sbError.append(
								"(32) " + "Favor de informar el Descuento correspondiente al Motido de descuento "
										+ " - factura " + factura + "\n");
					} else {
						if (strValues[31].toString().trim().equals("")) {
							invoice.setDescuento(0);
					...
					sbError.append(
							"(31) " + "Motivo de descuento con formato incorrecto " + " - factura " + factura + "\n");
					fError = true;
				}
			}

			// Leer los valores del Cliente cuando el RFC sea generico y el IdExtranjero
			// venga vacio

			if (invoice.getRfc() != null) {
				if (invoice.getRfc().trim().toUpperCase().equals("XEXX010101000")
						|| invoice.getRfc().trim().toUpperCase().equals("XAXX010101000")) {
					if (invoice.getIdExtranjero() == null || invoice.getIdExtranjero().trim().equals("")) {

						// Nombre de cliente
						if (strValues[32] == null) {
							sbError.append(
									"(33) Posicion Nombre de Cliente requerida (Null) - Factura " + factura + "\n");
							System.out.println(
									"(33) Posicion Nombre de Cliente requerida (Null) - Factura " + factura + "\n");
							fError = true;
						} else {
							...
							if (strValues[32].toString().trim().equals("")) {
								...
								if (validaDatoRELongitud(strValues[32].toString(), RE_CHAR_ALL, 250)) {
									...
									sbError.append("(33) Nombre de Cliente con formato incorrecto - Factura " + factura
											+ "\n");
									System.out.println("(33) Nombre de Cliente con formato incorrecto - Factura "
											+ factura + "\n");
									fError = true;
						...
						if (strValues[33] == null) {
							sbError.append(
									"(34) Posicion Nombre de Calle requerida (Null) - Factura " + factura + "\n");
							System.out.println(
									"(34) Posicion Nombre de Calle requerida (Null) - Factura " + factura + "\n");
							fError = true;
						} else {
							...
							if (strValues[33].toString().trim().equals("")) {
								...
								if (validaDatoRELongitud(strValues[33].toString(), RE_CHAR_ALL, 250)) {
									...
									sbError.append(
											"(34) Nombre de Calle con formato incorrecto - Factura " + factura + "\n");
									System.out.println(
											"(34) Nombre de Calle con formato incorrecto - Factura " + factura + "\n");
									fError = true;
						...
						if (strValues[34] == null) {
							...
							System.out
									.println("(35) Posicion Num Exterior requerida (Null) - Factura " + factura + "\n");
							fError = true;
						} else {
							...
							if (strValues[34].toString().trim().equals("")) {
								...
								if (validaDatoRELongitud(strValues[34].toString(), RE_CHAR_ALL, 250)) {
									...
									sbError.append(
											"(35) Num Exterior con formato incorrecto - Factura " + factura + "\n");
									System.out.println(
											"(35) Num Exterior con formato incorrecto - Factura " + factura + "\n");
									fError = true;
						...
						if (strValues[35] == null) {
							...
							System.out
									.println("(36) Posicion Num Interior requerida (Null) - Factura " + factura + "\n");
							fError = true;
						} else {
							...
							if (strValues[35].toString().trim().equals("")) {
								...
								if (validaDatoRELongitud(strValues[35].toString(), RE_CHAR_ALL, 250)) {
									...
									sbError.append(
											"(36) Num Interior con formato incorrecto - Factura " + factura + "\n");
									System.out.println(
											"(36) Num Interior con formato incorrecto - Factura " + factura + "\n");
									fError = true;
						...
						if (strValues[36] == null) {
							...
							if (strValues[36].toString().trim().equals("")) {
								...
								if (validaDatoRELongitud(strValues[36].toString(), RE_CHAR_ALL, 250)) {
									...
									System.out
											.println("(37) Colonia con formato incorrecto - Factura " + factura + "\n");
									fError = true;
						...
						if (strValues[37] == null) {
							...
							System.out.println(
									"(38) Posicion Codigo Postal requerida (Null) - Factura " + factura + "\n");
							fError = true;
						} else {
							...
							if (strValues[37].toString().trim().equals("")) {
								...
								if (validaDatoRELongitud(strValues[37].toString(), RE_NUMBER, 5)) {
									...
									sbError.append(
											"(38) Codigo Postal con formato incorrecto - Factura " + factura + "\n");
									System.out.println(
											"(38) Codigo Postal con formato incorrecto - Factura " + factura + "\n");
									fError = true;
						...
						if (strValues[38] == null) {
							...
							if (strValues[38].toString().trim().equals("")) {
								...
								if (validaDatoRELongitud(strValues[38].toString(), RE_CHAR_ALL, 250)) {
									...
									System.out.println(
											"(39) Localidad con formato incorrecto - Factura " + factura + "\n");
									fError = true;
						...
						if (strValues[39] == null) {
							...
							if (strValues[39].toString().trim().equals("")) {
								...
								if (validaDatoRELongitud(strValues[39].toString(), RE_CHAR_ALL, 250)) {
									...
									System.out.println(
											"(40) Municipio con formato incorrecto - Factura " + factura + "\n");
									fError = true;
						...
						if (strValues[40] == null) {
							...
							if (strValues[40].toString().trim().equals("")) {
								...
								if (validaDatoRELongitud(strValues[40].toString(), RE_CHAR_ALL, 250)) {
									...
									System.out
											.println("(41) Estado con formato incorrecto - Factura " + factura + "\n");
									fError = true;
			...
			for (int index = 42; index < strValues.length - 1; index++) {
				...
				if (posicionConcepto == 1) {
					...
					if (strItemConcepto.equals("")) {
						...
						sbError.append("(" + (posicion + 1) + ") " + "Cantidad requerida " + " en el Concepto "
								+ contadorConceptos + " - factura " + factura + "\n");
						// System.out.println("(" + (posicion+1) +") " + "Cantidad requerida " + " en el
						// Concepto " + contadorConceptos + " - factura " + factura + "\n");
					} else {
						// System.out.println("cantidad:" + strItemConcepto);
						if (validaDatoRE(strItemConcepto, RE_DECIMAL_QTY)) {
							// System.out.println("Cantidad: " + strItemConcepto + " de la factura " +
							// factura);
							if (fPermisoVector)
								vectorCantidad.add(strItemConcepto);
						} else {
							fPermisoVector = false;
							sbError.append("(" + (posicion + 1) + ") " + "Cantidad con formato incorrecto "
									+ " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
							System.out.println("(" + (posicion + 1) + ") " + "Cantidad con formato incorrecto "
									+ " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
						}
					...
					if (strItemConcepto.equals("")) {
						fPermisoVector = false;
						sbError.append("(" + (posicion + 1) + ") " + "UM requerida " + " en el Concepto "
								+ contadorConceptos + " - factura " + factura + "\n");
						System.out.println("(" + (posicion + 1) + ") " + "UM requerida " + " en el Concepto "
								+ contadorConceptos + " - factura " + factura + "\n");
					} else {
						if (validaDatoRELongitud(strItemConcepto, RE_CHAR_ALL, 250)) {
							...
							fPermisoVector = false;
							sbError.append("(" + (posicion + 1) + ") " + "UM con formato incorrecto "
									+ " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
							System.out.println("(" + (posicion + 1) + ") " + "UM con formato incorrecto "
									+ " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
						}
					}
					...
					if (strItemConcepto.equals("")) {
						...
						if (strItemConcepto != null && strItemConcepto.trim().length() > 0
								&& strItemConcepto.length() <= 1500) {
							// System.out.println("Concepto Expedicion: " + strItemConcepto + " de la
							// factura " + factura);
							if (fPermisoVector)
								vectorDesc.add(strItemConcepto.toUpperCase());
						} else {
							fPermisoVector = false;
							sbError.append("(" + (posicion + 1) + ") " + "Concepto Expedicion con formato incorrecto "
									+ " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
							System.out
									.println("(" + (posicion + 1) + ") " + "Concepto Expedicion con formato incorrecto "
											+ " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
						}
					}
					...
					if (strItemConcepto.equals("")) {
						fPermisoVector = false;
						sbError.append("(" + (posicion + 1) + ") " + "Precio Unitario requerido " + " en el Concepto "
								+ contadorConceptos + " - factura " + factura + "\n");
						// System.out.println("(" + (posicion+1) +") " + "Precio Unitario requerido " +
						// " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
					} else {
						// System.out.println("precioUnitario:" + strItemConcepto);
						if (validaDatoRE(strItemConcepto, RE_DECIMAL_NEGATIVO)) {
							// System.out.println("Precio Unitario: " + strItemConcepto + " de la factura "
							// + factura);
							if (fPermisoVector)
								vectorPrecioUnitario.add(Double.parseDouble(strItemConcepto));
						} else {
							fPermisoVector = false;
							sbError.append("(" + (posicion + 1) + ") " + "Precio Unitario con formato incorrecto "
									+ " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
							System.out.println("(" + (posicion + 1) + ") " + "Precio Unitario con formato incorrecto "
									+ " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
						}
					...
					if (strItemConcepto.equals("")) {
						// System.out.println("APLICA IVA: " + strItemConcepto + " de la factura " +
						// factura);
						if (fPermisoVector)
						...
						if (validaDatoRE(strItemConcepto, RE_NUMBER)) {
							...
							if (strItemConcepto.equals("1")) {
								...
								fPermisoVector = false;
								sbError.append("(" + (posicion + 1) + ") " + "APLICA IVA con formato incorrecto "
										+ " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
								System.out.println("(" + (posicion + 1) + ") " + "APLICA IVA con formato incorrecto "
										+ " en el Concepto " + contadorConceptos + " - factura " + factura + "\n");
							}
						} else {
							fPermisoVector = false;
			...
			if (posicionConcepto != 5 && posicionConcepto != 4) {
				sbError.append(
						"El Concepto " + contadorConceptos + ", está incompleto" + " en la factura " + factura + "\n");
				System.out.println(
						"El Concepto " + contadorConceptos + ", está incompleto" + " en la factura " + factura + "\n");
				fError = true;
			} else {
				if (fPermisoVector) {
					...
					if (vectorCantidad != null && vectorUM != null && vectorDesc != null && vectorPrecioUnitario != null
							...
							&& vectorPrecioUnitario.size() == vectorAplicaIVA.size()) {
			...
			if (fError) {
				...
				try {
					System.out.println("AntesfacturaOK - " + factura);
					/*
					 * System.out.println("fserie: " + invoice.getSerie() + "\t" + "fformaPago: " +
					 * invoice.getFormaPago() + "\t" + "fsubtotal: " + invoice.getSubTotal() + "\t"
					 * + "ftotal: " + invoice.getTotal() + "\t" + "ftipoComprobante: " +
					 * invoice.getTipoFormato() + "\t" + "fmetodoPago: " + invoice.getMetodoPago() +
					 * "\t" + "flugarExpedicion: " + invoice.getLugarExpedicion() + "\t" +
					 * "fnumCtaPago: " + invoice.getNumCtaPago() + "\t" + "emisorRFC: " +
					 * fiscalEntity.getTaxID() + "\t" + "emisorNombre: " +
					 * fiscalEntity.getFiscalName() + "\t" + "emisorCalle: " +
					 * fiscalEntity.getAddress().getStreet().toUpperCase() + "\t" +
					 ...
					 * "\t" + "regimenfiscal: " + invoice.getRegimenFiscal() + "\t" + "recptorRFC: "
					 * + invoice.getRfc() + "\t" + "recptorNombre: " + invoice.getName() + "\t" +
					 * "recptorcalle: " + invoice.getCalle() + "\t" + "recptornoExterior: " +
					 * invoice.getExterior() + "\t" + "recptornoInterior: " + invoice.getInterior()
					 * + "\t" + "recptorColonia: " + invoice.getColonia() + "\t" +
					 * "recptorlocalidad: " + invoice.getLocalidad() + "\t" + "recptorreferencia: "
					 * + invoice.getReferencia() + "\t" + "recptormunicipio: " +
					 * invoice.getMunicipio() + "\t" + "recptorestado: " + invoice.getEstado() +
					 * "\t" + "recptorpais: " + invoice.getPais() + "\t" + "recptorcodigoPostal: " +
					 * invoice.getCodigoPostal() + "\t" + "iva: " + invoice.getIva() + "\t" +
					 ...
					 * "getEmail: " + invoice.getEmail() + "\t" + "feCalle: " +
					 * fiscalEntity.getAddress().getStreet() + "\t" + "feNExterior: " +
					 * fiscalEntity.getAddress().getExternalNumber() + "\t" + "feIExterior: " +
					 * fiscalEntity.getAddress().getInternalNumber() + "\t" + "feIColonia: " +
					 * fiscalEntity.getAddress().getNeighborhood() + "\t" + "feIRef: " +
					 * fiscalEntity.getAddress().getReference() + "\t" + "feIRegion: " +
					 * fiscalEntity.getAddress().getRegion() + "\t" + "feIEstado: " +
					 * fiscalEntity.getAddress().getState().getName() + "\t" + "feIPais: " +
					 * fiscalEntity.getAddress().getState().getCountry().getName() + "\t" +
					 * "feIZipCod: " + fiscalEntity.getAddress().getZipCode() + "\t" + "\n");
					 */
					/*
					 * for(int iSub=0; iSub<vectorCantidad.size(); iSub++){
					 * System.out.println("conceptocantidad: " + vectorCantidad.get(iSub) + " um: "
					 * + vectorUM.get(iSub) + " descripcion: " + vectorDesc.get(iSub) +
					 * " preciounitario: " + vectorPrecioUnitario.get(iSub) + " aplicaiva: " +
					 * vectorAplicaIVA.get(iSub) + "\n");
					 * 
					 * }
					 */ // all fields debug
					System.out.println("Antes de crearFac");
					 ...
					 * catch (webServiceResponseException ex) { facturasError+=1;
					 * sb.append("Factura: " + factura +
					 * " -- Lista de Errores (WSResponseException): " + "\n" + ex.getMessage() +
					...
					// invoice.setSbError(sbError);
					invoice.setSbError(
							new StringBuilder("Factura: " + factura + " --Exception: " + e.getMessage() + "\n"));
					listIn.add(invoice);
				}
			 ...
			 * "Facturas procesadas: 0 -- Facturas correctas: 0 -- Facturas Incidencias: 0"
			 * ); mV.addObject("resultados",
			 * "Lista de Errores (Exception General - Global): " + "\n" + e.getMessage() +
			 * "\n"); return mV;
			 */
		}
		return sbErrorFile;
	...
	public void crearFactura(int renglon) throws Exception {
		...
		// //System.out.println("XML devuelto por generaXML.guarda: " +
		// this.invoice.getByteArrXMLSinAddenda());
		// }else{
		...
		while (continuarIntentando == true) {
			...
			try {
				...
				if (contIntentos <= 5) {
					...
					continuarIntentando = false;
					throw new Exception(
							"Documento timbrado. Problema al actualizar secuencia de folio en la base de datos");
				}
			}
		}

		/*
		 * Codigo de timbrado generaXML.timbraFactura(nameFile, invoice);
		 * 
		 * cFDIssued.setFolioSAT(generaXML.getUUID()); cFDIssued.setIsCFDI(1);
		 * 
		 * 
		 ...
		 * 
		 * boolean continuarIntentando=true; int contIntentos=0;
		 * while(continuarIntentando==true) { contIntentos++; try {
		 * openJpaManager.update(open); continuarIntentando=false; }catch(Exception e) {
		 * if(contIntentos<=5) { continuarIntentando=true; Thread.sleep(1000); } else {
		 * continuarIntentando=false; throw new
		 * Exception("Documento timbrado. Problema al actualizar secuencia de folio en la base de datos"
		 * ); } } }
		 * 
		 * continuarIntentando=true; contIntentos=0; while(continuarIntentando==true) {
		 * contIntentos++; try {
		 * cFDIssued.setFolioInterno(String.valueOf(open.getSequence_value()));
		 * cFDIssued=cFDIssuedManager.update(cFDIssued); continuarIntentando=false; }
		 * catch (Exception e) { if(contIntentos<=5) { continuarIntentando=true;
		 * Thread.sleep(1000); } else { continuarIntentando=false; throw new
		 * Exception("Documento timbrado. Problema al actualizar comprobante en la base de datos"
		 * ); } } }
		 */
		 ...
		 * 
		 * sb.append("RENGLON: " + renglon + " -- " + ".... -- ERROR: " +
		 * ex.getMessage() + "\n"); numeroFacturasError++;
		 * System.out.println("webServiceResponseException: " + ex.getMessage()); }
		 * catch (Exception e) {
		 * 
		 * sb.append("RENGLON: " + renglon + " -- " + ".... -- ERROR: " + e.getMessage()
		 * + "\n"); numeroFacturasError++; System.out.println("Exception Gral: " +
		 * e.getMessage()); }
		 */
	}
