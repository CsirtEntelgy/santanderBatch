...
@Component
public class GeneraDivisasXML_Masivo {
	/*private Logger logger = Logger.getLogger(GeneraDivisasXML_Masivo.class);
  ...
  //Banderas para comprobar que existan algunos atributos de Santander:Movimientos
	...
	@Autowired
	private GeneraArchivoDivisas_Masivo generaXML;
	@Autowired
	...
	private UserManager userManager;
   	...
   	public GeneraDivisasXML_Masivo() {
	 ...
	 * @param nameFile
	 */
	public GeneraDivisasXML_Masivo(String nameFile) {
	...
	public void processControlFile(String urlWSTimbrado){
		...
		try{
			...
			//FileOutputStream fileStatus = new FileOutputStream(facturacionDivisasProceso + "STATUS_PROCESS_" + nProceso + ".TXT");
			FileOutputStream userlog = null;
			FileOutputStream fileStatus = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasProcess.txt");
			fileStatus.write(("Status del proceso bash massiveDivisasProcess.sh" + "\n").getBytes("UTF-8"));
			while((strLine = br.readLine()) != null){
				...
				if(arrayRenglon.length>1){
					...
					File fileTXT = new File(facturacionDivisasProceso + arrayRenglon[1] + "/" + nameFileExcel);
					if(!fileTXT.exists()){
						fileStatus.write(("El archivo " + nameFileExcel + " no se encuentra en la ruta " + facturacionDivisasProceso + arrayRenglon[1] + "/" + "\n").getBytes("UTF-8"));
						userlog = new FileOutputStream(facturacionDivisasSalida + arrayRenglon[1] + "/LOG" + arrayRenglon[1] + ".TXT",true);
						userlog.write(("El archivo " + nameFileExcel + " no se encuentra en la ruta " + facturacionDivisasProceso + arrayRenglon[1] + "/" + "\r\n").getBytes("UTF-8"));
					}else{
						//Crear directorio en ruta ../facturacion/salida/
						/*File fileDirectory = new File(facturacionDivisasSalida + arrayRenglon[1] + "/");
						...
						//String strAbsolutePathXML = properties.getPathFacturacionDivisasSalida() + arrayRenglon[1] + "/" + "XML" + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
						String strAbsolutePathBD = facturacionDivisasProceso + arrayRenglon[1] + "/" + "BD" + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
						//String strAbsolutePathODM = properties.getPathFacturacionDivisasOndemand() + "ODM-" + nameFileExcel.substring(0, nameFileExcel.indexOf("."));
						//String strAbsolutePathINC = properties.getPathFacturacionDivisasSalida() + arrayRenglon[1] + "/" + "INC" + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
						...
						FileInputStream fisTxt = new FileInputStream(facturacionDivisasProceso + arrayRenglon[1] + "/" + nameFileExcel);
										...
										for(int index=0; index<listIn.size(); index++){
											if(listIn.get(index).getByteArrXMLSinAddenda() != null){
												...
												if(strDescripcion.toLowerCase().trim().equals("ok") && strIdRespuesta.equals("1")) {
													...
													String strAbsolutePathOndemand = facturacionDivisasOndemand + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + "/";
													...
													salidaBD.write(this.buildRowBDFile(listIn.get(index), strUUID, nameFileExcel, idUsuario, idArea).getBytes("UTF-8"));
						...
						String strAbsolutePathXML = facturacionDivisasSalida + arrayRenglon[1] + "/" + "XML" + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
						String strAbsolutePathINC = facturacionDivisasSalida + arrayRenglon[1] + "/" + "INC" + nameFileExcel.substring(0, nameFileExcel.indexOf(".")) + ".TXT";
			...
			ex.printStackTrace();
			System.out.println("Exception massiveDivisasProcess:" + ex.getMessage());
			
			try {
				//FileOutputStream fileError = new FileOutputStream(facturacionDivisasProceso + "ERROR_PROCESS_" + nProceso + ".TXT");
				FileOutputStream fileError = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasProcessError.txt");
				...
				System.out.println("Exception al crear massiveDivisasProcessError.txt:" + e1.getMessage());
			}		
		}
	}
	
	public String buildRowBDFile(Invoice_Masivo invoiceM, String strUUID, String strSourceFileName, String idUsuario, String idArea){		
			...
			try{
				...
				/*if(!strValues[strValues.length-1].trim().toUpperCase().equals("FINFACTURA")){
						...
						if(invoice.getRfc().trim().equals("")) {
							...
							if(!invoice.getRfc().toUpperCase().equals("XEXX010101000")){
	                        	...
	                        	if(strValues[6] == null){
									...
									if(strValues[6].toString().trim().length() == 0){
						...
						if(!strValues[16].toString().trim().equals("")){										
	                    	if(validaDatoRE(strValues[16].toString().trim(), RE_DECIMAL)) {
								...
								if(!hashIvas.containsKey(Integer.parseInt(tasa))) {												
									...
									//System.out.println("DescriptionIVA: " + hashIvas.get(tasa));
								}
							} else {											
					...
					if(!strValues[30].toString().trim().equals("")){
						if(strValues[30].toString().trim().toLowerCase().equals("compra") || strValues[30].toString().trim().toLowerCase().equals("venta") ){
							invoice.setTipoOperacion(strValues[30].toString().toLowerCase().trim());											
							//System.out.println("tipoOperacion: " + invoice.getTipoOperacion());				                        
					...
					if(strValues[31].toString().trim().equals("")){
						...
						if(strValues[32].toString().trim().equals("")){										
							...
							if(validaDatoRE(strValues[32].toString().trim(), RE_DECIMAL_NEGATIVO)){	
								
								if(Double.parseDouble(strValues[32].toString().trim()) > 0){
									fError = true;
				                	sbError.append("(32) " + "Favor de informar el Motivo de descuento correspondiente al Descuento " + " - factura " + factura + "\n");
								}else{
									invoice.setDescuento(Double.parseDouble(strValues[32].toString().trim()));
								}
						...
						if(strValues[31].toString().trim() != null && strValues[31].toString().trim().length() > 0 && strValues[31].toString().trim().length() <= 1500){											
							invoice.setMotivoDescuento(strValues[31].toString().trim());
							
							//Descuento
							if(strValues[32].toString().trim().equals("")){										
								invoice.setDescuento(0);
								fError = true;
			                	sbError.append("(33) " + "Favor de informar el Descuento correspondiente al Motido de descuento " + " - factura " + factura + "\n");
							}else{
								if(validaDatoRE(strValues[32].toString().trim(), RE_DECIMAL_NEGATIVO)){											
							 ...
							 sbError.append("(32) " + "Motivo de descuento con formato incorrecto " + " - factura " + factura + "\n");											 
							 fError = true;
		                 }
					}
					...
					for(int index=33; index<strValues.length-1; index++ ){
					...
					if(fError){
						...
						try{
							...
							*/
							System.out.println("APLICAIVA:" + String.valueOf(invoice.getSiAplicaIva()) + "***");
	...
	public void crearFactura(int renglon)throws Exception {
		...
		Date date=Calendar.getInstance().getTime();
		 ...
		 //try {
			...
			String nameFile = generaXML.generaXMLHandler(invoice, fe, fecha);
