...
public class UtilCFDIFormatoUnicoDivisas {
	...
	public CfdiComprobanteFiscal fillComprobanteFUDivisasNew(String[] linea){
		...
		if (linea[1] == null || linea[1].trim().equals("")) {
			...
			fiscalEntity.setTaxID(linea[1]);
			fiscalEntity = fiscalEntityManager.findByRFCA(fiscalEntity);
			comp.getEmisor().setRfc(linea[1].trim());
		}
...
//				comp.setFormaPago(linea[2].trim());
//			}
			comp.setFormaPago(linea[3].trim());
//			try {
...
//			}
		...
		if(linea[5] == null || linea[5].trim().equals("")){
			...
			try{
				comp.setDescuento(new BigDecimal(linea[5].trim()));
		...
		if (linea[7] == null || linea[7].trim().equals("")) {
			...
			comp.setTipoCambio(linea[7].trim());			
		}

		/* Tipo formato posicion 8 -- row 8 */
		if (linea[8] == null || linea[8].trim().equals("")) {
			comp.setTipoDeComprobante("I");
		} else {
			comp.setTipoDeComprobante(linea[8].trim());
		}
		...
		if (linea[9] == null || linea[9].trim().equals("")) {
			...
			comp.setMetodoPago(linea[9].trim());
		}

		// pos 10
		/* Regimen fiscal */
		if (linea[10] == null || linea[10].trim().equals("")) {
			comp.getEmisor().setRegimenFiscal("");
		} else {
			comp.getEmisor().setRegimenFiscal(linea[10].trim());
		}
		...
		if (linea[11] == null) {
			...
			if (linea[11].trim().equals("")) {
				comp.setCustomerRfcCellValue("");
			} else {
				comp.setCustomerRfcCellValue(linea[11].trim());
			}
		}
		...
		if (linea[12] == null || linea[12].trim().equals("")) {
			...
			comp.getReceptor().setNombre(linea[12].trim());
		}
		// pos 13
		/* ID extranjero*/
		if (linea[13] == null || linea[13].trim().equals("")) {
			...
			String strIDExtranjero = linea[13].trim();
		...
		if (linea[14] == null || linea[14].trim().equals("")) {
			...
			comp.getReceptor().setUsoCFDI(linea[14].trim());
		}
		// Pos 15
		/*NumRegIDTrib */
		if (linea[15] == null || linea[15].trim().equals("")) {
			...
			comp.getReceptor().setNumRegIdTrib(linea[15].trim());
		}
		...
		if (linea[16] == null || linea[16].trim().equals("")) {
			...
			comp.setNumeroCuenta(linea[16].trim());
		}
		...
		if (linea[18] == null || linea[18].trim().equals("")) {
			...
			comp.getAddenda().getInformacionEmision().setCodigoCliente(linea[18].trim());
		}
		...
		if (linea[19] == null || linea[19].trim().equals("")) {
			...
			comp.getAddenda().getInformacionEmision().setContrato(linea[19].trim());
		}
		// pos 20
		/* Periodo */
		...
		if (linea[21] == null || linea[21].trim().equals("")) {
			...
			comp.getAddenda().getInformacionEmision().setCentroCostos(linea[21].trim());
		}
		...
		if (linea[22] == null || linea[22].trim().equals("")) {
			...
			comp.getAddenda().getCampoAdicional().put("DESCRIPCIÓN CONCEPTO", linea[22].trim());
		}
		
		// pos 23
		/* Iva */
		if(linea[23] == null || linea[23].trim().equals("")){
			comp.setIvaCellValue("");
		}else{
			comp.setIvaCellValue(linea[23].trim());
		}
		// pos 24
		/* tipo addenda */
		comp.getAddenda().setInmuebles(new CfdiAddendaInmuebles());
		if(linea[24] != null){
			if(linea[24].toString().contains(".")){
				System.out.println("*** response Dentro IF AMDA: " + linea[24].toString());
				String words[] = linea[24].toString().split("\\.");
				linea[24] = words[0];
				System.out.println("*** response Dentro IF despues AMDA: " + linea[24].toString());
			}
			comp.setTipoAddendaCellValue(linea[24].toString());
			System.out.println("tipoAddenda:" + linea[24].toString());
			if (validaDatoRE(linea[23].toString().trim(), RE_DECIMAL)) {
				String strTipoAddenda = linea[24].toString();
				...
				if (linea[33] == null) {
					...
					comp.getAddenda().getInformacionPago().setNombreBeneficiario(linea[33].toString().trim());
				}
				...
				if (linea[34] == null) {
					...
					comp.getAddenda().getInformacionPago()
							.setInstitucionReceptora(linea[34].toString().trim());
				}
				//numero de cuenta
				if (linea[35] == null) {
					...
					if (linea[35].toString().trim().equals("")) {
						comp.getAddenda().getInformacionPago().setNumeroCuenta(linea[35].toString().trim());
					} else {
						comp.getAddenda().getInformacionPago().setNumeroCuenta(linea[35].toString().trim());
					}
				}
				...
				if (linea[36] == null) {
					...
					if (linea[36].toString().trim().equals("")) {
						comp.getAddenda().getInformacionPago().setNumProveedor(linea[36].toString().trim());
					} else {
						comp.getAddenda().getInformacionPago().setNumProveedor(linea[36].toString().trim());
					}
				}
				...
				if (linea[30] == null) {
					...
					if (linea[30].toString().trim().equals("")) {
						...
						comp.setCostCenter(linea[30].trim());
					}
				}
...
//					comp.getAddenda().getInformacionPago().setNumProveedor("");
					...
					//email
					if (linea[25].toString() == null) {
						comp.getAddenda().getInformacionPago().setEmail("");
					} else {
						// email
						if (!linea[25].toString().trim().equals("")) {
							if (validaDatoRE(linea[25].toString().trim(), RE_MAIL)) {
								comp.getAddenda().getInformacionPago().setEmail(linea[25].toString().trim());
							} else {
								comp.getAddenda().getInformacionPago().setEmail("");
							}
						} else {
					...
					if (linea[26] == null) {
						...
						if (!linea[26].toString().trim().equals("")) {
							comp.getAddenda().getInformacionPago()
								.setCodigoISOMoneda(linea[26].toString().trim().toUpperCase());
						} else {
							comp.getAddenda().getInformacionPago().setCodigoISOMoneda("");
						}
					}
				...
				if (strTipoAddenda.equals("1")) {
					...
					if (linea[27] == null) {
						...
						if (linea[27].toString().trim().equals("")) {
							...
							comp.getAddenda().getInformacionPago().setOrdenCompra(linea[27].toString().trim());
						}
					}
					//posicion compra
					if (linea[28] == null) {
						...
						if (linea[27].toString().trim().equals("")) {
							...
							comp.getAddenda().getInformacionPago().setPosCompra(linea[28].toString().trim());
						}
					}
					...
					if (linea[29] == null) {
						...
						if (linea[29].toString().trim().equals("")) {
							...
							comp.getAddenda().getInformacionPago().setCuentaContable(linea[29].toString().trim());
						}
					}
					...
					if (linea[31] == null) {
						...
						if (linea[31].toString().trim().equals("")) {
							...
							comp.getAddenda().getInmuebles().setNumContrato(linea[31].toString().trim());
						}
					}
					//fecha vencimiento
					if (linea[32] == null) {
						...
						if (linea[32].toString().trim().equals("")) {
							comp.getAddenda().getInmuebles().setFechaVencimiento("");
						} else {
							comp.getAddenda().getInmuebles().setFechaVencimiento(linea[32].toString().trim());
						}
					}
		...
		if(linea[38] ==  null){
			...
			comp.setNoAutorizacion(linea[38].toString().trim());
		}
		// pos 38 y 39
		
		/* Tipo de operacion */
		comp.setComplemento(new CfdiComplemento());
		if (!linea[37].toString().trim().equals("")) {
			if (linea[37].toString().trim().toLowerCase().equals("compra")
					|| linea[37].toString().trim().toLowerCase().equals("venta")) {
				comp.getComplemento().setDivisaTipoOperacion(linea[37].toString().toLowerCase().trim());
			} else {
		...
		if (linea[39] == null) {
			...
			comp.getComplemento().getTimbreFiscalDigital().setUuid(linea[39].toString().trim());
			comp.setCfdiRelacionados(new CfdiRelacionado());
			if (linea[40] == null) {
				comp.getCfdiRelacionados().setTipoRelacion("");
			} else {
				comp.getCfdiRelacionados().setTipoRelacion(linea[40].toString().trim());
