...
@Controller
public class MassiveDonatReadController {
	...
	public static String facturacionDonatEntrada="/salidas/masivo/donat/facturacion/entrada/";
	public static String facturacionDonatProceso="/salidas/masivo/donat/facturacion/proceso/";
	public static String facturacionDonatSalida="/salidas/masivo/donat/facturacion/salida/";
	public static String facturacionDonatOndemand="/salidas/masivo/donat/facturacion/ondemand/";
	
	public MassiveDonatReadController(){
		
	}
	
	public void createIdFileProcess(){			
		try{	
			List<Massive> listRequest = this.getRequests(0, 2);
			
			FileOutputStream fileStatus = new FileOutputStream(facturacionDonatProceso + "massiveDonatRead.txt");
			fileStatus.write("Status del proceso bash massiveDonatRead.sh\n".getBytes("UTF-8"));
			...
			/*for(int index=0; index<totalProcesos; index++){
				listFilesIdReport.add(new FileOutputStream(properties.getPathFacturacionDonatEntrada() + "IDFILEPROCESS_" + String.valueOf(index) + ".TXT"));
			}*/
			
			
			FileOutputStream fileProcess = new FileOutputStream(facturacionDonatEntrada + "IDFILEPROCESS.TXT");
			...
			if (listRequest != null && listRequest.size()>0){			
				...
				for ( Massive mObj : listRequest ){
					...
					File fileDirectory = new File(facturacionDonatSalida + mObj.getFilename() + "/");
					...
					userlog = new FileOutputStream(facturacionDonatSalida + mObj.getFilename() + "/LOG" + mObj.getFilename() + ".TXT");
			...
			e.printStackTrace();
			System.out.println("Exception massiveDonatRead:" + e.getMessage());
			try {
				FileOutputStream fileError = new FileOutputStream(facturacionDonatProceso + "massiveDonatReadError.txt");
				fileError.write((e.getMessage()).getBytes("UTF-8"));
				fileError.close();
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
				System.out.println("Exception al crear massiveDonatReadError.txt:" + e.getMessage());
			}			
		}		
	}
