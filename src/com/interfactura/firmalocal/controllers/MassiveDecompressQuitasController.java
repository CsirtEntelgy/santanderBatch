...
@Controller
public class MassiveDecompressQuitasController {
	@Autowired
	...
	public void readIdFileProcess(){
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(PathQuitasProceso + "massiveDecompressQuitas.txt");
			fileStatus.write(("Status del proceso bash massiveDecompressQuitas.sh" + "\n").getBytes("UTF-8"));
			while((strLine = br.readLine()) != null){
				if(!strLine.trim().equals("")){
					String [] arrayValues = strLine.trim().split("\\|");
					File fileZIP = new File(PathQuitasEntrada + arrayValues[1] + ".ZIP");
					if(fileZIP.exists()){
						...
						fileStatus.write(("El archivo " + arrayValues[1] + ".ZIP no se encuentra en la ruta " + PathQuitasEntrada + "\n").getBytes("UTF-8"));
						userlog = new FileOutputStream(PathQuitasSalida + arrayValues[1] + "/LOG" + arrayValues[1] + ".TXT",true);
						userlog.write(("El archivo " + arrayValues[1] + ".ZIP no se encuentra en la ruta " + PathQuitasEntrada + "\r\n").getBytes("UTF-8"));
					}					
				}			
				counter++;
			}			
			...
			e.printStackTrace();
			System.out.println("Exception massiveDecompressQuitas:" + e.getMessage());
			try {
				//FileOutputStream fileError = new FileOutputStream(properties.getPathReportesProceso() + "ERROR_DECOMPRESS_" + nProceso + ".TXT");
				FileOutputStream fileError = new FileOutputStream(PathQuitasProceso + "massiveDecompressQuitasError.txt");
				...
				System.out.println("Exception al crear massiveDecompressQuitasError.txt:" + e.getMessage());
			}			
		}
	}
	...
	public String decompress(String strNameFile) throws Exception{
	   ...
	   //FileInputStream(argv[0]);
         FileInputStream(PathQuitasEntrada + strNameFile + ".ZIP");
         ZipInputStream zis = new ZipInputStream(new BufferedInputStream(fis));
         ZipEntry entry;
         //Comprobar existencia del directorio
         File fileDirectory = new File(PathQuitasProceso + strNameFile + "/");
         ...
         try{
	         while((entry = zis.getNextEntry()) != null) {
	            ...
	            if(!entry.isDirectory()){
		            ...
		            FileOutputStream fos = new FileOutputStream(PathQuitasProceso + strNameFile + "/" + nameFile);
        	 ...
        	 return "Se encontraron nombres incorrectos en archivo(s), dentro de " + PathQuitasEntrada + strNameFile + ".ZIP - caracteres validos: letras (sin acentos, dieresis y ñ), numeros, puntos, espacios en blanco, guiones y guiones bajos; ej. nombreArchivo - 1_Y.xlsx";
         }catch(Exception e){
        	 throw e; 
         }
         zis.close();     
