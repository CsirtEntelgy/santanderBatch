...
@Controller
public class MassiveDivisasInfoInvoiceController {
	@Autowired
	...
	public void readIdFileProcess(){
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasInfoInvoice.txt");
			fileStatus.write(("Status del proceso bash massiveDivisasInfoInvoice.sh" + "\n").getBytes("UTF-8"));
			while((strLine = br.readLine()) != null){
				if(!strLine.trim().equals("")){
					String [] arrayValues = strLine.trim().split("\\|");
					File fileBD = new File(facturacionDivisasProceso + arrayValues[1] + "/" + "BD" + arrayValues[1] + ".TXT");
					if(fileBD.exists()){
						readBD(fileBD,arrayValues[1]);
						fileStatus.write(("Extraccion de informacion exitosa, del archivo BD" + arrayValues[1] + ".TXT" + "\n").getBytes("UTF-8"));
					}else{
					
						//No existe el archivo BD para la solicitud de FM
						System.out.println("No existe archivo BD para la solicitud de FM: " + arrayValues[1]);
					
						fileStatus.write(("El archivo BD" + arrayValues[1] + ".TXT no ha sido leido en la ruta " + facturacionDivisasProceso + arrayValues[1] + "/" + "\n").getBytes("UTF-8"));
					}					
				}			
				counter++;
			}			
			...
			e.printStackTrace();
			System.out.println("Exception massiveDivisasInfoInvoice:" + e.getMessage());
			try {
				//FileOutputStream fileError = new FileOutputStream(properties.getPathReportesProceso() + "ERROR_DECOMPRESS_" + nProceso + ".TXT");
				FileOutputStream fileError = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasInfoInvoiceError.txt");
				...
				System.out.println("Exception al crear massiveDivisasInfoInvoiceError.txt:" + e.getMessage());
			}			
		}
	}
	...
	public void readBD(File fileBD, String fileName) throws Exception{
		...
		FileOutputStream salidaInfoFacturas = new FileOutputStream(new File(facturacionDivisasProceso + fileName + "/FACTURASDIARIO" + fileName + ".TXT"));
		...
		while((strLine = br.readLine()) != null){
			if(!strLine.trim().equals("")){
				...
				salidaInfoFacturas.write(extractionInfo(facturacionDivisasOndemand + fileName + "/" + "fac-" + arrayValuesBd[20] + ".xml", arrayValuesBd, fileName, counter).getBytes("UTF-8"));
