...
@Controller
public class InvoiceController_NC 
{	
	@Autowired
	...
	private Logger logger = Logger.getLogger(InvoiceController_NC.class);
	...
	public synchronized void processingInvoices(String idProceso,
			String fileNames, String fecha, String urlWebService, String isNC) 
	{
					...
					if(path.equals("null"))
					{
						...
						this.processing(
								new File(properties.getPathDirProECB())
									...
									fileNames, fecha, idProceso, urlWebService);
					}
					else 
					{
						
						if(!isNC.equals("")){
							logger.info("Paso 2.- Procesando Notas de Credito por Tareas: "+idProceso);
							...
							if(file.exists()&&file.length()>0)
							{
								...
								{			
									...
									{	
								    	...
								    	logger.info("Paso 2.- Procesando Notas de Credito (byte Inicio)"+byteStart);
								    	logger.info("Paso 2.- Procesando Notas de Credito (byte Inicio)"+byteEnd);
								    	/*
								    	...
								    	this.processing(true, setF, lstTipoCambio, lstCampos22, 
												lstSeal, lstIva, transf, val, byteStart, byteEnd, args[1], cont, idProceso, fecha, fileNames, urlWebService, isNC);
									}
				...
				else 
				{
					...
					if(idProceso.equals("-1"))
					{
						...
						FiltroParam filter =new FiltroParam(new String[]{"XML","INC","backUp"}, 
								new String[]{"CFDLZELAVON","CFDOPOPICS","CFDCONFIRMINGFACTURAS","CFDFACTORAJEFACTURAS"});
						this.processing(
...
class Thread1 implements Runnable{
		...
		public synchronized  void run() {
			...
			if (this.flag){				
				InvoiceController_NC.this.processingLines(this.pathT, this.byteStartT,  this.byteEndT,  this.contT,
				...
				InvoiceController_NC.this.processingLines2(this.pathT, this.byteStartT,  this.byteEndT,  this.contT,
						this.setFT,  this.lstTipoCambioT, this.lstCampos22T, 
						this.lstSealT,  this.lstIvaT,  this.valT, this.idProcesoT, this.fechaT, this.fileNamesT);				
			}
		}
	...
	*/
	...
	public void processing(boolean isECB,
			...
			long byteStart, long byteEnd, String path,long cont, String idProceso, String fecha, String fileNames, String urlWebService, String isNC) 
	{
		File file=new File(path);
		if(file.length()>0)
		{
			...
			{
				if(!isNC.equals("")){
					logger.debug("Procesando Notas de Credito");
					xmlNC.setNameFile(file.getName());
					xmlNC.setLstFiscal(lstFiscal);
					xmlNC.setLstSeal(lstSeal);
					xmlNC.setCampos22(lstCampos22);
					xmlNC.setTipoCambio(lstTipoCambio);
					xmlNC.setTransf(transf);
					xmlNC.setValidator(val);
					xmlNC.setUrlWebService(urlWebService);
					xmlNC.convierte(byteStart, byteEnd, path, cont, idProceso, fecha, fileNames);
				}else{
