...
@Controller
public class MassiveReportBuilderDivisasController {
	...
	public MassiveReportBuilderDivisasController(){
	...
	public void readIdReportProcess(String nProceso){
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(reportesDivisasProceso + "reportBuilder_" + nProceso + ".txt");
			...
			e.printStackTrace();
			System.out.println("Exception reportBuilderDivisas:" + e.getMessage());
			try {
				FileOutputStream fileError = new FileOutputStream(reportesDivisasProceso + "reportBuilderDivisasError_" + nProceso + ".txt");
				...
				e1.printStackTrace();
				System.out.println("Exception al crear reportBuilderDivisasError_" + nProceso + ".txt:" + e.getMessage());
			}
	...
	//	}
		...
		File fileOut = new File( reportesDivisasProceso + ID + "/" + ID + "REPORT.TXT" );
		//File fileOut = new File( PATH_OUT );
		//aFile = new RandomAccessFile(properties.getPathReportesProceso() + ID + "/" + ID + "FACTURAS.TXT", "r");
		aFile = new RandomAccessFile( reportesDivisasProceso + ID + "/" + ID + "CFDS.TXT","r");
