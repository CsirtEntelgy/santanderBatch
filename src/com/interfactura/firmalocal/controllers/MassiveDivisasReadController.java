...
@Controller
public class MassiveDivisasReadController {
	...
	public static String facturacionDivisasEntrada="/salidas/masivo/divisas/facturacion/entrada/";
	public static String facturacionDivisasProceso="/salidas/masivo/divisas/facturacion/proceso/";
	public static String facturacionDivisasSalida="/salidas/masivo/divisas/facturacion/salida/";
	public static String facturacionDivisasOndemand="/salidas/masivo/divisas/facturacion/ondemand/";
	
	public MassiveDivisasReadController(){
		
	}
	
	public void createIdFileProcess(){			
		try{	
			List<Massive> listRequest = this.getRequests(0, 1);
			
			FileOutputStream fileStatus = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasRead.txt");
			fileStatus.write("Status del proceso bash massiveDivisasRead.sh\n".getBytes("UTF-8"));
			...
			/*for(int index=0; index<totalProcesos; index++){
				listFilesIdReport.add(new FileOutputStream(properties.getPathFacturacionDivisasEntrada() + "IDFILEPROCESS_" + String.valueOf(index) + ".TXT"));
			}*/
			
			
			FileOutputStream fileProcess = new FileOutputStream(facturacionDivisasEntrada + "IDFILEPROCESS.TXT");
			...
			if (listRequest != null && listRequest.size()>0){			
				...
				for ( Massive mObj : listRequest ){
					...
					File fileDirectory = new File(facturacionDivisasSalida + mObj.getFilename() + "/");
					...
					userlog = new FileOutputStream(facturacionDivisasSalida + mObj.getFilename() + "/LOG" + mObj.getFilename() + ".TXT");
			...
			e.printStackTrace();
			System.out.println("Exception massiveDivisasRead:" + e.getMessage());
			try {
				FileOutputStream fileError = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasReadError.txt");
				fileError.write((e.getMessage()).getBytes("UTF-8"));
				fileError.close();
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
				System.out.println("Exception al crear massiveDivisasReadError.txt:" + e.getMessage());
			}			
		}		
	}
