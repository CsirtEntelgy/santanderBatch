...
@Controller
public class MassiveReadQuitasController {
	@Autowired
	...
	public static String PathQuitasEntrada="/salidas/masivo/quitas/facturacion/entrada/";
	public static String PathQuitasProceso="/salidas/masivo/quitas/facturacion/proceso/";
	public static String PathQuitasSalida="/salidas/masivo/quitas/facturacion/salida/";
	public static String PathQuitasOndemand="/salidas/masivo/quitas/facturacion/ondemand/";
	
	public MassiveReadQuitasController(){
		
	}
	
	public void createIdFileProcess(){			
		try{	
			List<Massive> listRequest = this.getRequests(0, 4);
			
			FileOutputStream fileStatus = new FileOutputStream(PathQuitasProceso + "massiveReadQuitas.txt");
			fileStatus.write("Status del proceso bash massiveReadQuitas.sh\n".getBytes("UTF-8"));
			...
			FileOutputStream fileProcess = new FileOutputStream(PathQuitasEntrada + "IDFILEPROCESS.TXT");
			...
			if (listRequest != null && listRequest.size()>0){			
				...
				for ( Massive mObj : listRequest ){
					...
					File fileDirectory = new File(PathQuitasSalida + mObj.getFilename() + "/");
					...
					userlog = new FileOutputStream(PathQuitasSalida + mObj.getFilename() + "/LOG" + mObj.getFilename() + ".TXT");
			...
			try {
				FileOutputStream fileError = new FileOutputStream(PathQuitasProceso + "massiveReadQuitasError.txt");
				fileError.write((e.getMessage()).getBytes("UTF-8"));
				fileError.close();
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
				System.out.println("Exception al crear massiveReadQuitasError.txt:" + e.getMessage());
			}			
		}		
	}
