...
@Controller
public class MassiveDonatDepuraController {
	@Autowired
	...
	public MassiveDonatDepuraController(){
		
	}
	public void depura(){
		
		try{
			FileOutputStream fileStatus = new FileOutputStream(facturacionDonatProceso + "massiveDonatDepura.txt");
			
			//Depura procesados
			List<Massive> listMassive = massiveManager.findByStatus(2, 2); 
			
			fileStatus.write("Status del proceso bash massiveDonatDepura.sh\n".getBytes("UTF-8"));
			if(listMassive.size()>0){
				...
				for(String strIdName:listIdsName){
					...
					String strPathProceso = facturacionDonatProceso + arrayValues[1] + "/";
					...
					String strPathSalida = facturacionDonatSalida + arrayValues[1] + "/";
					...
					String strPathOndemand = facturacionDonatOndemand + arrayValues[1] + "/";
					...
					deleteFiles(arrayValues[1], facturacionDonatSalida, false);
					
					//Eliminar archivos del directorio ../ondemand/
					deleteFiles(arrayValues[1], facturacionDonatOndemand, false);
					
					//Eliminar archivos del directorio ../entrada/
					deleteFiles(arrayValues[1], facturacionDonatEntrada, false);
			...
			List<Massive> listMassiveNoProcesados = massiveManager.findByDepuraStatusNoProcesado(2); 
			...
			e.printStackTrace();
			System.out.println("Exception massiveDonatDepura:" + e.getMessage());
			try {
				FileOutputStream fileError = new FileOutputStream(facturacionDonatProceso + "massiveDonatDepuraError.txt");
				fileError.write((e.getMessage()).getBytes("UTF-8"));
				fileError.close();
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
				System.out.println("Exception al crear massiveDonatDepuraError.txt:" + e.getMessage());
			}
