...
@Controller
public class MassiveInfoConceptsQuitasController {
	@Autowired
	...
	public void readIdFileProcess(){
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(PathQuitasProceso + "massiveInfoConceptQuitas.txt");
			fileStatus.write(("Status del proceso bash massiveInfoConceptQuitas.sh" + "\n").getBytes("UTF-8"));
			while((strLine = br.readLine()) != null){
				if(!strLine.trim().equals("")){
					String [] arrayValues = strLine.trim().split("\\|");
					File fileBD = new File(PathQuitasProceso + arrayValues[1] + "/IDSDIARIO"  + arrayValues[1] + "QUERY.TXT");
					if(fileBD.exists()){
						...
						fileStatus.write(("El archivo QUERY" + arrayValues[1] + ".TXT no ha sido leido en la ruta " + PathQuitasProceso + arrayValues[1] + "/" + "\n").getBytes("UTF-8"));
					}					
				}			
				counter++;
			}			
			...
			try {
				...
				FileOutputStream fileError = new FileOutputStream(PathQuitasProceso + "massiveInfoConceptError.txt");
	...
	public void readBD(File fileBD, String fileName) throws Exception{
		...
		FileOutputStream salidaInfoConceptos = new FileOutputStream(new File(PathQuitasProceso + fileName + "/CONCEPTOSDIARIO" + fileName + ".TXT"));
