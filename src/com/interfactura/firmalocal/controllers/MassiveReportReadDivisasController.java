...
@Controller
public class MassiveReportReadDivisasController {
	...
	public static String reportesDivisasEntrada="/salidas/masivo/divisas/reportes/entrada/";
	public static String reportesDivisasProceso="/salidas/masivo/divisas/reportes/proceso/";
	public static String reportesDivisasSalida="/salidas/masivo/divisas/reportes/salida/";
	
	public MassiveReportReadDivisasController(){
	...
	public void createIdReportProcess(String nProcesos){			
		try{	
			List<MassiveReport> listRequestReport = this.getRequests(0, 1);
			
			FileOutputStream fileStatus = new FileOutputStream(reportesDivisasProceso + "reportReadDivisas.txt");
			fileStatus.write("Status del proceso bash reportReadDivisas.sh\n".getBytes("UTF-8"));
			...
			for(int index=0; index<totalProcesos; index++){
				listFilesIdReport.add(new FileOutputStream(reportesDivisasEntrada + "IDREPORTPROCESS_" + String.valueOf(index+1) + ".TXT"));
			}
			...
			if (listRequestReport != null && listRequestReport.size()>0){			
				...
				for ( MassiveReport mObj : listRequestReport ){
					...
					File fileDirectory = new File(reportesDivisasProceso + String.valueOf(mObj.getId()) + "/");
					...
					FileOutputStream fileQuery = new FileOutputStream(reportesDivisasProceso + mObj.getId() + "/" + mObj.getId() + "QUERY.sql");
					...
					FileOutputStream fileMaxQuery = new FileOutputStream(reportesDivisasProceso + mObj.getId() + "/" + mObj.getId() + "MAXQUERY.sql");
			...
			try {
				FileOutputStream fileError = new FileOutputStream(reportesDivisasProceso + "reportReadDivisasError.txt");
				fileError.write((e.getMessage()).getBytes("UTF-8"));
				fileError.close();
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
				System.out.println("Exception al crear reportReadDivisasError.txt:" + e.getMessage());
			}			
		}		
	}
