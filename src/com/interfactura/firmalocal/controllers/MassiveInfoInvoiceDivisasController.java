...
@Controller
public class MassiveInfoInvoiceDivisasController {
	...
	public void readIdFileProcess(String file, String time){
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(PathDivisasProceso + "massiveInfoInvoiceDivisas.txt");
			fileStatus.write(("Status del proceso bash massiveInfoInvoiceDivisas.sh" + "\n").getBytes("UTF-8"));
			
			File fileProcesoTxt = new File(PathDivisasProceso + fileName + "/" + fileName + ".TXT" );
			if(fileProcesoTxt.exists()){
				File fileBD = new File(PathDivisasProceso + fileName + "/" + "BD" + fileName + ".TXT");
				if(fileBD.exists()){
					...
					fileStatus.write(("El archivo BD" + fileName + ".TXT no ha sido leido en la ruta " + PathDivisasProceso + fileName + "/" + "\n").getBytes("UTF-8"));
				}
			} else
				fileStatus.write(("El archivo " + fileName + ".TXT no ha sido leido en la ruta " + PathDivisasProceso + fileName + "/" + "\n").getBytes("UTF-8"));
			...
			try {
				...
				FileOutputStream fileError = new FileOutputStream(PathDivisasProceso + "massiveInfoInvoiceError.txt");
	...
	public void readBD(File fileBD, String fileName) throws Exception{
		...
		FileOutputStream salidaInfoFacturas = new FileOutputStream(new File(PathDivisasProceso + fileName + "/FACTURASDIARIO" + fileName + ".TXT"));
		...
		while((strLine = br.readLine()) != null){
			if(!strLine.trim().equals("")){
				...
				salidaInfoFacturas.write(extractionInfo(PathDivisasOndemand + fileName + "/" + "fac-" + arrayValuesBd[20] + ".xml", arrayValuesBd, fileName, counter).getBytes("UTF-8"));
