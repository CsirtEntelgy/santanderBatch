...
@Controller
public class MassiveInfoConceptsArrendadoraController {
	@Autowired
	...
	public void readIdFileProcess(String file, String time){
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(PathArrendadoraProceso + "massiveInfoConceptQuitas.txt");
			fileStatus.write(("Status del proceso bash massiveInfoConceptQuitas.sh" + "\n").getBytes("UTF-8"));
			
			File fileProcesoTxt = new File(PathArrendadoraProceso + fileName + "/" + fileName + ".TXT" );
			if(fileProcesoTxt.exists()){
				File fileBD = new File(PathArrendadoraProceso + fileName + "/IDSDIARIO"  + fileName + "QUERY.TXT");
				if(fileBD.exists()){
					readBD(fileBD,fileName);
					fileStatus.write(("Extraccion de informacion exitosa, del archivo " + fileName + "QUERY.TXT" + "\n").getBytes("UTF-8"));
				}else{
				
					//No existe el archivo BD para la solicitud de FM
					System.out.println("No existe archivo QUERY para la solicitud de FM: " + fileName);
				
					fileStatus.write(("El archivo QUERY" + fileName + ".TXT no ha sido leido en la ruta " + PathArrendadoraProceso + fileName + "/" + "\n").getBytes("UTF-8"));
				}	
			}
			...
			try {
				...
				FileOutputStream fileError = new FileOutputStream(PathArrendadoraProceso + "massiveInfoConceptError.txt");
	...
	public void readBD(File fileBD, String fileName) throws Exception{
		...
		FileOutputStream salidaInfoConceptos = new FileOutputStream(new File(PathArrendadoraProceso + fileName + "/CONCEPTOSDIARIO" + fileName + ".TXT"));
