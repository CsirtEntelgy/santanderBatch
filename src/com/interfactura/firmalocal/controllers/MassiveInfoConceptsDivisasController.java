...
@Controller
public class MassiveInfoConceptsDivisasController {
	@Autowired
	...
	public void readIdFileProcess(String file, String time){
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(PathDivisasProceso + "massiveInfoConceptQuitas.txt");
			...
			if(fileProcesoTxt.exists()){
				File fileBD = new File(PathDivisasProceso + fileName + "/IDSDIARIO"  + fileName + "QUERY.TXT");
				if(fileBD.exists()){
					...
					fileStatus.write(("El archivo QUERY" + fileName + ".TXT no ha sido leido en la ruta " + PathDivisasProceso + fileName + "/" + "\n").getBytes("UTF-8"));
				}	
			}
			...
			try {
				...
				FileOutputStream fileError = new FileOutputStream(PathDivisasProceso + "massiveInfoConceptError.txt");
	...
	public void readBD(File fileBD, String fileName) throws Exception{
		...
		FileOutputStream salidaInfoConceptos = new FileOutputStream(new File(PathDivisasProceso + fileName + "/CONCEPTOSDIARIO" + fileName + ".TXT"));
