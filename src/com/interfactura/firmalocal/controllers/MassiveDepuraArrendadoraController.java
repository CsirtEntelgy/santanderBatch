...
@Controller
public class MassiveDepuraArrendadoraController {
	@Autowired
	...
	public void depura(String file, String time){
		...
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(PathArrendadoraProceso + "massiveDepuraArrendadora.txt");
			
			//Depura procesados
			
			List<Massive> listMassive = massiveManager.findByStatus(2, 4); 
			
			fileStatus.write("Status del proceso bash massiveDepuraArrendadora.sh\n".getBytes("UTF-8"));
			...
			if(fileProcesoTxt.exists()){
					...
					String strPathProceso = PathArrendadoraProceso + fileName + "/";
					...
					String strPathSalida = PathArrendadoraSalida + fileName + "/";
					...
					String strPathOndemand = PathArrendadoraOndemand + fileName + "/";
					...
					deleteFiles(fileName, PathArrendadoraSalida, false);
					
					//Eliminar archivos del directorio ../ondemand/
					deleteFiles(fileName, PathArrendadoraOndemand, false);
					
					//Eliminar archivos del directorio ../entrada/
					deleteFiles(fileName, PathArrendadoraEntrada, false);														
			...
			try {
				FileOutputStream fileError = new FileOutputStream(PathArrendadoraProceso + "massiveDepuraArrendadoraError.txt");
