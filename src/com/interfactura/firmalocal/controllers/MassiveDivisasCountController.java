...
@Controller
public class MassiveDivisasCountController {
	...
	public MassiveDivisasCountController(){
	...
	public void checkOK(){
		try{					
			...
			//FileOutputStream fileStatus = new FileOutputStream(properties.getPathFacturacionDivisasProceso() + "STATUS_COUNT_" + nProceso + ".TXT");
			FileOutputStream userlog = null;
			FileOutputStream fileStatus = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasCount.txt");
			fileStatus.write("Status del proceso bash massiveDivisasCount.sh\n".getBytes("UTF-8"));
			String strID;
			int counter = 0;
			while((strID = br.readLine()) != null){
				...
				if(arrayRenglon.length>1){
					Massive massive = massiveManager.getById(Long.parseLong(arrayRenglon[0].trim()));
						...
						fileStatus.write(("Facturacion masiva " + arrayRenglon[1] + " generada exitosamente\n").getBytes("UTF-8"));
			...
			e.printStackTrace();
			System.out.println("Exception massiveDivisasCount:" + e.getMessage());
			try {
				//FileOutputStream fileError = new FileOutputStream(properties.getPathFacturacionDivisasProceso() + "ERROR_COUNT_" + nProceso + ".TXT");
				FileOutputStream fileError = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasCountError.txt");
				...
				System.out.println("Exception al crear massiveDivisasCountError.txt:" + e.getMessage());
			}			
		}
	}
	...
	public void compressZip(String fileName) throws Exception{
		String zipFile = facturacionDivisasSalida + "CIFRAS" + fileName + ".ZIP";
		
		String srcDir = facturacionDivisasSalida  + fileName + "/";	
