...
@Controller
public class MassiveInfoInvoiceQuitasController {
	...
	public void readIdFileProcess(){
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(PathQuitasProceso + "massiveInfoInvoiceQuitas.txt");
			fileStatus.write(("Status del proceso bash massiveInfoInvoiceQuitas.sh" + "\n").getBytes("UTF-8"));
			while((strLine = br.readLine()) != null){
				if(!strLine.trim().equals("")){
					String [] arrayValues = strLine.trim().split("\\|");
					File fileBD = new File(PathQuitasProceso + arrayValues[1] + "/" + "BD" + arrayValues[1] + ".TXT");
					if(fileBD.exists()){
						...
						fileStatus.write(("El archivo BD" + arrayValues[1] + ".TXT no ha sido leido en la ruta " + PathQuitasProceso + arrayValues[1] + "/" + "\n").getBytes("UTF-8"));
					}					
				}			
				counter++;
			}			
			...
			try {
				...
				FileOutputStream fileError = new FileOutputStream(PathQuitasProceso + "massiveInfoInvoiceError.txt");
	...
	public void readBD(File fileBD, String fileName) throws Exception{
		...
		FileOutputStream salidaInfoFacturas = new FileOutputStream(new File(PathQuitasProceso + fileName + "/FACTURASDIARIO" + fileName + ".TXT"));
		...
		while((strLine = br.readLine()) != null){
			if(!strLine.trim().equals("")){
				...
				salidaInfoFacturas.write(extractionInfo(PathQuitasOndemand + fileName + "/" + "fac-" + arrayValuesBd[20] + ".xml", arrayValuesBd, fileName, counter).getBytes("UTF-8"));
