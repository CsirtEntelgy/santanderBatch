...
@Controller
public class MassiveDivisasInfoConceptsController {
	@Autowired
	...
	public void readIdFileProcess(){
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasInfoConcept.txt");
			fileStatus.write(("Status del proceso bash massiveDivisasInfoConcept.sh" + "\n").getBytes("UTF-8"));
			while((strLine = br.readLine()) != null){
				if(!strLine.trim().equals("")){
					String [] arrayValues = strLine.trim().split("\\|");
					File fileBD = new File(facturacionDivisasProceso + arrayValues[1] + "/IDSDIARIO"  + arrayValues[1] + "QUERY.TXT");
					if(fileBD.exists()){
						...
						fileStatus.write(("El archivo QUERY" + arrayValues[1] + ".TXT no ha sido leido en la ruta " + facturacionDivisasProceso + arrayValues[1] + "/" + "\n").getBytes("UTF-8"));
					}					
				}			
				counter++;
			}			
			...
			e.printStackTrace();
			System.out.println("Exception massiveDivisasInfoConcept:" + e.getMessage());
			try {
				//FileOutputStream fileError = new FileOutputStream(properties.getPathReportesProceso() + "ERROR_DECOMPRESS_" + nProceso + ".TXT");
				FileOutputStream fileError = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasInfoConceptError.txt");
				...
				System.out.println("Exception al crear massiveDivisasInfoConceptError.txt:" + e.getMessage());
			}			
		}
	}
	...
	public void readBD(File fileBD, String fileName) throws Exception{
		...
		FileOutputStream salidaInfoConceptos = new FileOutputStream(new File(facturacionDivisasProceso + fileName + "/CONCEPTOSDIARIO" + fileName + ".TXT"));
