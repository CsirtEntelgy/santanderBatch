...
@Controller
public class MassiveDepuraComplementoPagoController {
	@Autowired
	...
	public MassiveDepuraComplementoPagoController(){
		
	}
	public void depura(){
		
		try{
			FileOutputStream fileStatus = new FileOutputStream(PathFacturacionProceso + "massiveDepura.txt");
			
			//Depura procesados
			
			List<Massive> listMassive = massiveManager.findByStatus(2, 0); 
			
			fileStatus.write("Status del proceso bash massiveDepura.sh\n".getBytes("UTF-8"));
			if(listMassive.size()>0){
				...
				for(String strIdName:listIdsName){
					...
					String strPathProceso = PathFacturacionProceso + arrayValues[1] + "/";
					...
					String strPathSalida = PathFacturacionSalida + arrayValues[1] + "/";
					...
					String strPathOndemand = PathFacturacionOndemand + arrayValues[1] + "/";
					...
					deleteFiles(arrayValues[1], PathFacturacionSalida, false);
					
					//Eliminar archivos del directorio ../ondemand/
					deleteFiles(arrayValues[1], PathFacturacionOndemand, false);
					
					//Eliminar archivos del directorio ../entrada/
					deleteFiles(arrayValues[1], PathFacturacionEntrada, false);
			...
			List<Massive> listMassiveNoProcesados = massiveManager.findByDepuraStatusNoProcesado(0); 
			...
			try {
				FileOutputStream fileError = new FileOutputStream(PathFacturacionProceso + "massiveDepuraError.txt");
