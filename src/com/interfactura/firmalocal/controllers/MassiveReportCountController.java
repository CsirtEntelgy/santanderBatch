...
@Controller
public class MassiveReportCountController {
	@Autowired
	...
	public MassiveReportCountController(){
	...
	public void checkReportsOK(String nProceso){
		try{					
			...
			FileInputStream fsExcelsToProcess = new FileInputStream(reportesEntrada + "IDREPORTPROCESS_" + nProceso + ".TXT");
			DataInputStream in = new DataInputStream(fsExcelsToProcess);
			BufferedReader br = new BufferedReader(new InputStreamReader(in));
			
			FileOutputStream fileStatus = new FileOutputStream(reportesProceso + "reportCount_" + nProceso + ".txt");
			fileStatus.write("Status del proceso bash reportCount.sh\n".getBytes("UTF-8"));
			String strID;
			int counter=0;
			while((strID = br.readLine()) != null){
				MassiveReport massiveReport = massiveReportManager.getById(Long.parseLong(strID));
				Date date = new Date();
				//if(existFile(strID, reportesSalida)){					
			...
			e.printStackTrace();
			System.out.println("Exception reportCount:" + e.getMessage());
			try {
				FileOutputStream fileError = new FileOutputStream(reportesProceso + "reportCountError_" + nProceso + ".txt");
				fileError.write((e.getMessage()).getBytes("UTF-8"));
				fileError.close();
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
				System.out.println("Exception al crear reportCountError_" + nProceso + ".txt:" + e.getMessage());
			}			
		}
	}
