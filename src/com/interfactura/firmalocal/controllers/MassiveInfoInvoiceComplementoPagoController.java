...
@Controller
public class MassiveInfoInvoiceComplementoPagoController {
	...
	public void readIdFileProcess(){
		try{
			...
			FileOutputStream fileStatus = new FileOutputStream(PathFacturacionProceso + "massiveInfoInvoice.txt");
			fileStatus.write(("Status del proceso bash massiveInfoInvoice.sh" + "\n").getBytes("UTF-8"));
			while((strLine = br.readLine()) != null){
				if(!strLine.trim().equals("")){
					String [] arrayValues = strLine.trim().split("\\|");
					File fileBD = new File(PathFacturacionProceso + arrayValues[1] + "/" + "BD" + arrayValues[1] + ".TXT");
					if(fileBD.exists()){
						...
						fileStatus.write(("El archivo BD" + arrayValues[1] + ".TXT no ha sido leido en la ruta " + PathFacturacionProceso + arrayValues[1] + "/" + "\n").getBytes("UTF-8"));
					}					
				}			
				counter++;
			}			
			...
			try {
				...
				FileOutputStream fileError = new FileOutputStream(PathFacturacionProceso + "massiveInfoInvoiceError.txt");
	...
	public void readBD(File fileBD, String fileName) throws Exception{
		...
		FileOutputStream salidaInfoFacturas = new FileOutputStream(new File(PathFacturacionProceso + fileName + "/FACTURASDIARIO" + fileName + ".TXT"));
		...
		while((strLine = br.readLine()) != null){
			if(!strLine.trim().equals("")){
				...
				salidaInfoFacturas.write(extractionInfo(PathFacturacionOndemand + fileName + "/" + "fac-" + arrayValuesBd[20] + ".xml", arrayValuesBd, fileName, counter).getBytes("UTF-8"));
