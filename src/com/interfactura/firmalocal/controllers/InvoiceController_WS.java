...
import com.interfactura.firmalocal.xml.ecb.GeneraXML_ECBDSV3_3_WS;
...
@Controller
public class InvoiceController_WS {
	@Autowired
	private GeneraXML_ECBDS xmlECB;
	@Autowired
	private GeneraXML_ECBDSV3_3_WS xmlECBV3; // Clase para version 3.3 AMDA
	@Autowired
	...
	private Logger logger = Logger.getLogger(InvoiceController_WS.class);
...
//					            //file.delete();
								...
								File saveFile = new File(properties.getPathSalida() + "SAVECFD" + fecha + "-" + idProceso + ".TXT");
								String[] datosSave = null;
								String[] estatusSave = null;
								boolean encontrado = false;
								System.out.println("Nombre archivo SAVE: " + saveFile.getPath());
								if(saveFile.exists()) {
									System.out.println("Si existe");
									RandomAccessFile saveRead = new RandomAccessFile(saveFile, "r");
									datosSave = saveRead.readLine().split("\\|");
									estatusSave = saveRead.readLine().split("\\|");
									if(saveRead != null) {
							            saveRead.close();							            	
						            }
									if(datosSave != null && datosSave.length >3) {
										long offSet = Long.parseLong(datosSave[2]);
										long contSave = Long.parseLong(datosSave[0]);
										xmlECBV3.setOffSetComprobante(offSet);
										xmlECBV3.setProcessStarted(true);
										reader = new LineNumberReader(new FileReader(file));	
										//fileout=new File(properties.getPathDirProcesados()+file.getName());
							            ...
							            while ((line = reader.readLine()) != null) {
											...
											if(args!=null&&args.length>=5) {
												...
												if( estatusSave[0].equals("Termino")) {
													if(contSave < cont && args[1].equals(datosSave[3])) {
												    	encontrado = true;
											    	}else {
											    		System.out.println("La ejecucion se guardo en " + contSave + " el bloque" + cont + " se completo");
											    	}
												} else if(estatusSave[0].equals("Continua")) {
													if(contSave <= cont && args[1].equals(datosSave[3])) {
														encontrado = true;
											    	}else {
											    		System.out.println("La ejecucion se guardo en " + contSave + " el bloque" + cont + " se completo");
												...
												if ( encontrado ) {
													this.processing(true, setF, lstTipoCambio, lstCampos22, 
															lstSeal, lstIva, transf, val, byteStart, byteEnd, args[1],
															cont, idProceso, fecha, fileNames, urlWebService, numeroMalla);
												}
											}
										}
										...
										System.out.println("ERROR: los datos del archivo SAVECFD son incorrectos - " + saveFile.getPath());	
									}
								}else {
									System.out.println("No existe");
									//------------- PROCESO ORGINAL SIN COSAS PARA EL REINICIO ------------
									reader = new LineNumberReader(new FileReader(file));	
									//fileout=new File(properties.getPathDirProcesados()+file.getName());
