...
@Controller
public class MassiveDivisasDepuraController {
	@Autowired
	...
	public MassiveDivisasDepuraController(){
		
	}
	public void depura(){
		
		try{
			FileOutputStream fileStatus = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasDepura.txt");
			
			//Depura procesados
			List<Massive> listMassive = massiveManager.findByStatus(2, 1); 
			
			fileStatus.write("Status del proceso bash massiveDivisasDepura.sh\n".getBytes("UTF-8"));
			if(listMassive.size()>0){
				...
				for(String strIdName:listIdsName){
					...
					String strPathProceso = facturacionDivisasProceso + arrayValues[1] + "/";
					...
					String strPathSalida = facturacionDivisasSalida + arrayValues[1] + "/";
					...
					String strPathOndemand = facturacionDivisasOndemand + arrayValues[1] + "/";
					...
					deleteFiles(arrayValues[1], facturacionDivisasSalida, false);
					
					//Eliminar archivos del directorio ../ondemand/
					deleteFiles(arrayValues[1], facturacionDivisasOndemand, false);
					
					//Eliminar archivos del directorio ../entrada/
					deleteFiles(arrayValues[1], facturacionDivisasEntrada, false);
			...
			List<Massive> listMassiveNoProcesados = massiveManager.findByDepuraStatusNoProcesado(1); 
			...
			e.printStackTrace();
			System.out.println("Exception massiveDivisasDepura:" + e.getMessage());
			try {
				FileOutputStream fileError = new FileOutputStream(facturacionDivisasProceso + "massiveDivisasDepuraError.txt");
				fileError.write((e.getMessage()).getBytes("UTF-8"));
				fileError.close();
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
				System.out.println("Exception al crear massiveDivisasDepuraError.txt:" + e.getMessage());
			}
