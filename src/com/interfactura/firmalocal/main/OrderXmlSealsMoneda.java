...
public class OrderXmlSealsMoneda {
	...
	public static void main(String[] args) {
		...
		try{
			...
			hashObjetosXML = buildHashMap(pathInc + "XMLMERGE.TXT", args[1], args[2]);
			...
			while((line2 = bReader2.readLine()) != null){
				...
				if(line2.trim().length() > 0){
					if(hashObjetosXML.containsKey(line2.trim())){						
						...
						System.out.println("NO se encontro ArchivoSalidakey:" + line2.trim() + "-contador:" + counter);
					}
				}
	...
	public static HashMap<String, StringBuilder> buildHashMap(String pathXML, String lastAccountCFDMerge, String type){
		try{
			System.out.println("pathXML:" + pathXML);
			System.out.println("lastAccountCFDMerge:" + lastAccountCFDMerge);
			System.out.println("type:" + type);
			
			String [] strValuesType = type.trim().split("\\|");
			
			String [] strValuesMerge =lastAccountCFDMerge.trim().split("\\|");
					
			String lastAccountMerge = strValuesType[1] + "|" + strValuesMerge[7] + "|" + strValuesMerge[3];
					...
					if(firstTime){
						...
						if(line.substring(0, 4).equals("CFD|")){
							...
							*/
							...
							if(line.trim().startsWith("TIPO_DOCUMENTO|")){
								String [] strValues = line.split("\\|");
								strLastAccount = strValues[1] + "|" + strLastMoneda + "|" + strLastAccount; 
							}
